...
def load_glove(normalize):
    ...
    with open("glove-profession-data.pickle", "rb") as f:
            ...
            for i, gamma in enumerate(kernel2params[kernel_type]["gammas"]):
                for j, degree in enumerate(kernel2params[kernel_type]["degrees"]):
                    for k, alpha in enumerate(kernel2params[kernel_type]["alphas"]):
                        ...
                        RANK = 1
                        ws, advs, best_adv, best_score = solve_fantope_relaxation(X_kernel, Y, d=RANK, init_beta=None, device=args.device, out_iters=30000, in_iters_adv=1, in_iters_clf=1, batch_size=128, epsilon=0.025, noise_std=1e-3, lr=0.01, weight_decay=1e-5, momentum=0.0)
